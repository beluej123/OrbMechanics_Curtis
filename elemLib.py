...
def osculating_elements_of(position, reference_frame=None, gm_km3_s2=None):
    ...
    if gm_km3_s2 is None:
        raise ValueError(
            "Unable to calculate without a gm value. Please"
            " specify gm using the `gm_km3_s2` keyword argument"
        )
    ...
    return OsculatingElements(position_vec, velocity_vec, position.t, gm_km3_s2)
...
class OsculatingElements(object):
    ...
    def __init__(self, position, velocity, time, mu_km_s):
        if mu_km_s <= 0:
            raise ValueError(
                "`mu_km_s` (standard gravitational parameter "
...
def apoapsis_distance(p, e):
    if p.ndim == 0:
        ...
        return divide(
            p * (1 + e), 1 - e**2, out=repeat(inf, p.shape), where=e < (1 - 1e-15)
        )
...
def eccentric_anomaly(v, e, p):
    if e.ndim == 0:
        ...
        inds = e > 1  # hyperbolic
        E[inds] = normpi(
            2 * arctanh(tan(v[inds] / 2) / sqrt((e[inds] + 1) / (e[inds] - 1)))
        )
...
def test_OsculatingElements():
    """
    r1_vec, v1_vec taken from skyfield
    """

    r1_vec_au = np.array([-0.95300372, -0.29020189, -0.12581142])  # [au], equatorial
    v1_vec_au_day = np.array(
        [0.00518755, -0.01502928, -0.00651651]
    )  # [au/day], equatorial
    position = r1_vec_au * AU_  # convert to [km]
    velocity = v1_vec_au_day * (AU_ / DAY_S)  # convert to [km/s]
    time = datetime(2025, 3, 14, 0, 0, 0)
    mu_km_s = GM_SUN

    elem1 = OsculatingElements(
        position=position, velocity=velocity, time=time, mu_km_s=mu_km_s
    )
    print(f"vars(elem1):\n{vars(elem1)}")
    print(f"dir(elem1):\n{dir(elem1)}")
